...
def guardar():
    if not entrada_matriculanew.get() or not entrada_nombre.get()  or not direccion_var.get() or not carrera_var.get() or not sexo_var.get() or not entrada_fechanac.get() or not entrada_apellidos.get() or not etiqueta_imagen.image:
        ...
        try:
            ...
            fecha_nac=entrada_fechanac.get().strip()
            ...
            pdf.cell(0, 10, txt="Credencial de Estudiante", ln=1, align="C")
            pdf.ln(5)
            pdf.set_font("Arial", size=12)
            pdf.cell(0, 10, txt=f"Matrícula: {matricula}", ln=1)
            pdf.cell(0, 10, txt=f"Nombre: {nombre}", ln=1)
            pdf.cell(0, 10, txt=f"Apellidos: {apellidos}", ln=1)
            pdf.cell(0, 10, txt=f"Sexo: {sexo}", ln=1)
            pdf.cell(0, 10, txt=f"Dirección: {direccion}", ln=1)
            pdf.cell(0, 10, txt=f"Carrera: {carrera}", ln=1)
            pdf.cell(0, 10, txt=f"Fecha de Nacimiento: {fecha_nac}", ln=1)
            pdf.ln(1)

            # Insertar imagen si existe ruta
            if ruta:
                try:
                    pdf.image(ruta, x=150, y=30, w=40, h=40)
